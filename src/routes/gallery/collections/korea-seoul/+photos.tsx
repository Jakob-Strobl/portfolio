...
export const photoCollection: PhotoResource[] = getPhotosByCollection("korea-seoul");
...
export const PhotoCollection = (props: PhotoCollectionProps) => {
  ...
  return (
    <div class="flex flex-col gap-4">
      <h2 class="text-4xl text-white">Korea - Seoul</h2>
      <div class="grid grid-cols-1 lg:grid-cols-2 3xl:grid-cols-3 gap-4">
        {photos().map((photo) => (
          <GalleryPhoto
            collectionDirName="korea-seoul"
            photoId={photo.id}
            r2Key={photo.r2Key}
            dimensions={photo.dimensions}
            alt={`${photo.name.replace(/\.[^/.]+$/, "")} — Korea - Seoul`}
            variant="grid"
          />
        ))}
      </div>
